...
init_small_table(KatssCounter *counter, unsigned int kmer)
{
	...
	counter->table.small = s_malloc(((size_t)counter->capacity+1) * sizeof *counter->table.small);
...
init_medium_table(KatssCounter *counter, unsigned int kmer)
{
	...
	counter->table.medium = s_malloc(((size_t)counter->capacity+1) * sizeof *counter->table.medium);
	for(uint32_t i=0; i <= counter->capacity; i++) {
