...
struct KatssCounter {
	...
	_Atomic(uint64_t) total;        /** Total k-mers counted so far */
	union {
		_Atomic(uint64_t) *small;  /** for k<=12 use long to store large amounts */
		_Atomic(uint32_t) *medium; /** K>12 use 32bit to save memory */
